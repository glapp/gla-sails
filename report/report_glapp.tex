...
Being a lightweight virtualization technology, containerization get rid of the guest OS layer as in traditional virtual machine architecture.
Instead, applications are deployed on top of a container engine which runs on the host OS.
In addition, in containerization application binary are stored in a package along with the necessary libraries.
...
In contrasts, a container image used in our case study take up only a few hundred megabytes as it only includes the application binary and required libraries.
...
This component of the application facilitates users to interact with all the other components in the system. Since the whole back-end provides REST endpoints for all the features, platform is fairly relaxed in choosing front-end components. Angular was particularly chosen for two important reasons. Since we are using node.js for the server component, it helps a great deal in terms of productivity to be able to use the same language for other components. Team could easily switch developing features between the server and front-end. This can greatly aid to iterate faster. 

	Some of the benefits of using Angular framework are as follows. Data binding is the foremost advantage in the application, meaning any dynamic fields in the interface can be watched and automatically updated as the new data comes in. Directives for DOM manipulation and two way data binding makes a lot of known hardships a breeze with angular. Angular code can be very modular, reusable with easy to test features. Templating system provides great reusability of various components. We used Bower and NPM for package management, as they provide comprehensive set of libraries for all the application needs. Routing is built-in to angular, which makes a single page application possible. This can make an application run extremely smooth and fast, once its loaded on the browser. Directives are very useful to define custom HTML tags and reuse the most frequent features in the views. In addition to this, modularity, abstraction of services makes a strong case for Angular. Last but not least, it is supported and maintained by Google, which can guarantee a long term roadmap and future development of the framework.
