...
While both virtual machine and containerization are virtualization technologies that provision shared cloud computing resources, virtual machine focuses on virtualization of infrastructure resources and containerization focuses on virtualization of platform \todo{add reference}.
...
For the development of the MAPE loop we used the BURLAP\footnote{http://burlap.cs.brown.edu/} reinforcing learning library of Brown University. The work consisted in:
\begin{enumerate}
	...
	\item applying learning algorith, e.g. Q-Learning or SARSA
\end{enumerate}
Representing the current state resulted in a huge state space.
Considering that MDP learns from past rewards of getting to a specific state, we realized that our MDP wouldn't be efficient.
The main reason is that it would take too much exploring time until the system could begin to explote the knowledge aquired.
Every adaptation the MAPE would trigger, would also give the reward for the new state.
If we assume that in the normal operation of the system a rule violation should not be very frequently, then it could last years until the whole state-action space is explored, if at all.
A solution to this is using a function approximation.
With function approximation in place the system doesn't need anymore to explore all state-action pairs.
But instead, when a new pair is encountered it uses the same values of a similar state-action pair.
BURLAP in its current version 3 supports function approximation only for numeric representation of the state.
Since we have nominal values (strings) for the representation, an additional effort would be needed to use function approximation.
...
A further down side of rule-based adaptation is that the system doesn't react or has a undefined behaviour in case the MAPE encounters an unknown situation.
...
It retrieves everything through HTTP requests, like for example the infrastructure information, the application deployment information, the user-defined policies, and the performance metrics.
For instance, the infrastructure information includes information of the instances such as the service provider, instance location and size. Deployment information, on the other hand, refers to the information of each application components including on which containers they are currently deployed.
...
Classes in the package \texttt{ch.uzh.glapp.mdp} are implementation according to the BURLAP framework\footnote{http://burlap.cs.brown.edu/tutorials/bpl/p4.html\#qlearn}.
\begin{itemize}
...
As discussed in section 3.3.6, the MAPE allows custom learning algorithm to be used instead of the actual implemented rule-based adaptations.
...
In FERL, the action-value function, Q, is approximated as the negative free-energy of a restricted Boltzmann machine \todo{add reference} where a restricted Boltzmann machine is a type of the product of experts model.
In a restricted Boltzmann machine, binary random variables are represented as nodes in a bipartite graph.
The nodes belong to two disjointed set: visible state and action nodes and hidden nodes with weight on every edge connecting a visible node and a hidden node.
Free energy can be computed as an expected energy, which is determined by the weights, minus an entropy.
The negative free energy is used to approximate the Q-value ($Q_t = -F_t$).
