...
In the first implementation of MAPE, MDP as a reinforcement learning process was used to solve the optimization problem of finding the application deployment that fulfil the requirements specified in the policy.
...
To reduct the number of states, function approximation is needed.
...
More preciously, these attributes are the number of cells in an organ, the cloud service provider, region and hardware specification of the hosts that cells reside.
These features consist of discrete string values and as a result both SARSA and Q-Learning methods cannot be used.
However, since the provided function approximation methods from current BURLAP library does not handle string-valued feature, further study of appropriate function approximation and implementation are needed.
...
In the end, rule based logic is used in MAPE for determination of adaption action. 
